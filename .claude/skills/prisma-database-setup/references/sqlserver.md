...
Prisma defaults to the `dbo` schema for SQL Server. To target a different schema, use the `@@schema` attribute on your models and add `schemas` to the `datasource` block's `schemas` preview feature.
